...
GROUP BY
    skills_dim.skill_id
), avarage_salary AS (      -- When doing multiple CTE remember to group them like this "end of first CTE),new_cte_name AS()"
SELECT 
    skills_job_dim.skill_id,
    ROUND(AVG(job_postings_fact.salary_year_avg), 0) AS avg_salary --ROUND(collumn), number you want to round to).
FROM 
    job_postings_fact
INNER JOIN skills_job_dim ON job_postings_fact.job_id = skills_job_dim.job_id
INNER JOIN skills_dim ON skills_job_dim.skill_id = skills_dim.skill_id
...
/*
A more simple qurry that does the same thing
*/
